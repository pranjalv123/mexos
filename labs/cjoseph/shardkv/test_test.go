  ...
  for i := 0; i < ngroups; i++ {
    ...
    for j := 0; j < nreplicas; j++ {
      sa[i][j] = StartServer(gids[i], smh, ha[i], j)
	    DPrintf("test is starting server %d.%d", gids[i], j)
...
func TestBasic(t *testing.T) {
  ...
  ck := MakeClerk(smh)
	fmt.Printf("Made clerk!\n")
  ck.Put("a", "x")
	fmt.Printf("Just put 1st val!\n")
  v := ck.PutHash("a", "b")
  if v != "x" {
	  t.Fatalf("Puthash got wrong value, should 'x' was '%s'",v)
  }
  ov := NextValue("x", "b")
  if ck.Get("a") != ov {
