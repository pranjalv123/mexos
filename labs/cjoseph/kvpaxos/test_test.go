...
//	  log.Println("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!about to get")
    for i := 0; i < nservers; i++ {
      va[i] = cka[i].Get("b")
      if va[i] != va[0] {
	      t.Fatalf("mismatch on get b %s ",va)
      }
    }
  }
  ...
  const ncli = 5
    ...
    for cli := 0; cli < ncli; cli++ {
      ...
      go func(me int) {
        ...
        if ov != pv {
		t.Fatalf("wrong value for puthash(%s, 0); expected %s but got %s",key, pv, ov)
        }
	      log.Println("*****************passed puthash(",key,",0) ***************")
        ov = myck.PutHash(key, "1")
        pv = NextValue(pv, "0")
        if ov != pv {
		t.Fatalf("wrong value for  puthash (%s, 1); expected %s but got %s",key, pv, ov)
        }
	      log.Println("****************passed puthash(",key,",1)*****************")
        ov = myck.PutHash(key, "2")
        pv = NextValue(pv, "1")
        if ov != pv {
		t.Fatalf("wrong value for puthash(%s,2); expected %s",key, pv)
        }
	      log.Println("******************passed puthash(",key,",2)***************")
        nv := NextValue(pv, "2")
        time.Sleep(100 * time.Millisecond)
	      log.Println("********************about to get",key,"**************")
        if myck.Get(key) != nv {
		t.Fatalf("wrong value for get %s %s ",key, nv)
        }
	      log.Println("********************SUCCESSFUL get",key,"**************")
        if myck.Get(key) != nv {
          t.Fatalf("wrong value")
        }
	      log.Println("********************another SUCCESSFUL get",key,"**************")
        ok = true
      }(cli)
    }
	  log.Println("****I am sitting in a room, debugging this stupid lab*********")
    for cli := 0; cli < ncli; cli++ {
	    log.Println("lol", cli+1," of", ncli)
      x := <- ca[cli]
      if x == false {
        t.Fatalf("failure")
      }
...
func TestHole(t *testing.T) {
  ...
  for iters := 0; iters < 5; iters++ {
	  log.Println("starting iteration",iters)
    part(t, tag, nservers, []int{0,1,2,3,4}, []int{}, []int{})
    ...
    ck2.Put("q", "qq")
    check(t, ck2, "q", "qq")
	  log.Println("about to restore the partition")
