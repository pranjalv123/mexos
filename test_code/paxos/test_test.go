...
func TestPersistenceBasic(test *testing.T) {
	...
	if !decided || (value != trueValue) {
		test.Fatalf("Restarted server did not learn about missed instance")
	}
	fmt.Printf("\n\tPassed")
}

// Test that instances are not forgotten when majority partition is restarted
func TestPersistencePartition(test *testing.T) {
	if onlyBenchmarks || !runNewTests {
		return
	}
	runtime.GOMAXPROCS(4)
...
func TestPersistenceAllRestart(test *testing.T) {
	if onlyBenchmarks || !runNewTests {
		return
	}
	runtime.GOMAXPROCS(4)
...
func TestForget(test *testing.T) {
	if onlyBenchmarks || !runOldTests {
		return
	}
	runtime.GOMAXPROCS(4)
...
func TestManyForgetUnreliable(test *testing.T) {
	if onlyBenchmarks || !runOldTests {
		return
	}
	runtime.GOMAXPROCS(4)
...
//
func TestForgetMem(test *testing.T) {
	if onlyBenchmarks || !runOldTests {
		return
	}
	runtime.GOMAXPROCS(4)
...
func TestRPCCountRegular(test *testing.T) {
	if onlyBenchmarks || !runOldTests {
		return
	}
	runtime.GOMAXPROCS(4)
...
// Check that RPC counts aren't too high
func TestRPCCountPrePrepare(test *testing.T) {
	if onlyBenchmarks || !runOldTests {
		return
	}
	runtime.GOMAXPROCS(4)
...
//
func TestManyUnreliable(test *testing.T) {
	if onlyBenchmarks || !runOldTests {
		return
	}
	runtime.GOMAXPROCS(4)
